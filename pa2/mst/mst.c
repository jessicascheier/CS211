...
int main ( int argc, char* argv[] ) {
    ...
    parents[root] = root; // root is a random starting vertex 
    //parents[vertex] is changed from -1 to random numNode so that it is already marked as visited
    ...
    for (unsigned iter=0; iter<graphNodeCount-1; iter++) { // n - 1 edges
        ...
        for (graphNode_t source=0; source<graphNodeCount; source++) {
            if (parents[source] != -1) {
                /* ... */
                AdjacencyListNode* neighbor = &adjacencyList[source]; 
                while (neighbor != NULL) {
    ...
    /* ... */
    for (size_t j = 0; j < graphNodeCount; j++) {
