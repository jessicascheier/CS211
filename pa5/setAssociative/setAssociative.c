...
void accessData (
    ...
    int lru_index = 0; // least recently used line
    size_t lru_set_line = set[0].recent_use; // first line is the initial minimum
    for (int i = 1; i < E; i++) {
        if (set[i].recent_use < lru_set_line) {
            lru_index = i;
            lru_set_line = set[i].recent_use;
        }
    }
    ...
    set[lru_index].recent_use = program_counter;
