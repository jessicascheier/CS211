...
void accessData (
    ...
    if ( cache->occupancy == E ) {
        ...
        curr_line = cache->front; // dequeue from front of FIFO
        cache->front = curr_line->next_cache_line; // cut off front, free it
        free(curr_line); // prevent memory leak?
        cache->occupancy = (cache->occupancy)-1; // update occupancy
        (*eviction_count)++; // record an eviction
    }

    // Due to cache miss, enqueue cache line, and update occupancy
    cache_line_t* enqueue_line = (cache_line_t*) malloc(sizeof(cache_line_t));
    enqueue_line->tag = tag;
    enqueue_line->next_cache_line = NULL;

    if (cache->front == NULL) { // cache empty
        cache->front = enqueue_line;
        cache->back = enqueue_line;
        cache->occupancy = (cache->occupancy)+1;
    } else { // not empty
        cache->back->next_cache_line = enqueue_line; // this way, cache->back is guaranteed not null
        cache->back = enqueue_line;
        cache->occupancy = (cache->occupancy)+1;
    }
